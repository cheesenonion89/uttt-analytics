...
# plotly.tools.set_credentials_file(username='molig', api_key='InQv6RKZ00uHRwx5lMKJ')
...
print(get_depth_traces_by_round('final1', 1))
layout = go.Layout(title='Depth per Round - Final 1', width=1024, height=800)
data = []
for index in range(0, 100):
    data.append(go.Box(
        name=str(index),
        y=get_depth_traces_by_round('final1', index)
    ))

plot = go.Figure(data=data, layout=layout)

py.image.save_as(plot, filename='bars.png')
